...
public class GameNoUI {
	...
	private void userTurn()
	{
		...
		int choice = askForInput(); //The index of the card the user picks
	...
	{
		//Conditions:
		boolean sameNum = player.getCard(choice).getNumber() == centerCard.getNumber();
		boolean sameColor = player.getCard(choice).getColor().equalsIgnoreCase(centerCard.getColor());
		boolean wild = player.getCard(choice) instanceof WildCard || player.getCard(choice) instanceof DrawFour;
		if(sameNum || sameColor || wild)
		{
			return true;
		}
	...
	{
		System.out.println("\nIt is your turn\n" + user);
		System.out.println("The center card is: " + centerCard);
		if (centerCard instanceof WildCard || centerCard instanceof DrawFour) {
	...
	private void ai(Hand player)
	{
		...
		for(int i = 0; i < player.getSize(); i++)
		{
			if((player.getCard(i).getColor().equalsIgnoreCase(centerCard.getColor())
					&& player.getCard(i).getNumber() != 999) || (player.getCard(i).getNumber() == 
